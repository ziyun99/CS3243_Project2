...
class NewExtractor(FeatureExtractor):
    ...
    def getManhattanDistances(self, pos, ghosts): 
        g = []
        for ghost in ghosts:
            ghost = util.manhattanDistance(pos, ghost.getPosition());
            g.append(ghost);
        return g;
    ...
    def getFeatures(self, state, action):
        ...
        if scaredGhost:
            distanceToClosestScaredGhost = min(self.getManhattanDistances(pos, scaredGhost))
            if activeGhost:
                distanceToClosestActiveGhost = min(self.getManhattanDistances(pos, activeGhost))
            else:
                distanceToClosestActiveGhost = 10	
            features["capsules"] = capsulesLeft

            if distanceToClosestScaredGhost < 5 and distanceToClosestActiveGhost > 1:
                features["#-of-ghosts-1-step-away"] = 0
                features["eats-food"] = 0.0
                features["closest-ghost"] = float(distanceToClosestScaredGhost) / (walls.width * walls.height);
