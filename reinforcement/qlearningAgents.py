...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        return self.random.random.choice(maxAction)
    ...
    def getAction(self, state):
        ...
        if(randomAction):
            ...
            # action = self.random.random.choice(legalActions)
            action = util.random.choice(legalActions)
        else:
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.counter[state, action] = (1.0 - self.alpha) * current + self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        fe = self.featExtractor;
        features = fe.getFeatures(state, action);
        qvalue = 0
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        next_state = self.getLegalActions(nextState)
        maxqnext = -99999
        for act in next_state:
        ...
        self.counter[(state,action)] += self.alpha * diff 
        fe = self.featExtractor;
        features = fe.getFeatures(state,action)
        for feature in features.keys():
            value = features[feature];
